...
| **Cloudflare R2 now exists and the app bucket works — two gaps remain.** Superseded the original "no R2 account at all" blocker on 2026-09-16, verified live against the real account, not assumed. **Working:** real credentials in `.env.local`; `HeadBucket` on the app bucket; a full presigned **PUT → GET → DELETE** round-trip server-side; `/api/health` returns `r2: true`. **Gap 1 — CORS is not configured on the app bucket.** An `OPTIONS` preflight with `Origin: http://localhost:3000` (and with a Vercel origin) returns 403 and no `Access-Control-Allow-Origin`. The browser uploads directly to R2 (`01-hld.md` §9), so every real user upload still fails with an opaque network error until CORS allows the app's own origins. The server-side round-trip above does **not** exercise CORS and must not be read as evidence it works. **Gap 2 — the app token cannot read the backup bucket.** `HeadBucket`/`ListObjectsV2` on `R2_BACKUP_BUCKET` return **403 AccessDenied** (not 404, so the bucket exists; the token lacks the grant). `.env.example` and D17 both specify read-only access for exactly one caller, `backup.verify` — the daily cron that asserts a real nightly dump exists, which AGENTS.md classes a **P1** ("the only recovery point"). It fails on every run until the grant is added. Still unverified either way: a real browser PUT, thumbnail generation and its EXIF-absence check, the orphan sweep actually deleting, and the GitHub Actions backup write. | Voola | Gap 1 blocks every real upload (Build 06/08 photos, Build 09 bill copies); Gap 2 blocks `backup.verify` | 2026-09-16 |
