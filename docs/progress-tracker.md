...
| 3.13 Sentry | ➖ | **Removed 2026-09-17 by D49** — out of scope for v1. It never reported an event: no org, DSN or auth token was ever created, so it was inert in every environment. Dependency, config files and `SENTRY_DSN` are gone. **The redaction filter and its 11 tests are kept** (`lib/observability/redact.ts`), caller-less, as the control any future error sink must use. |
...
| ~~No Sentry org, DSN or auth token~~ **Resolved 2026-09-17 by D49** — Sentry removed entirely, so nothing is blocked on it. There is no error-tracking service and no source-map upload. | — | Voola | 2026-09-09 |
...
| `lib/jobs/handlers/inventory.reconcile.ts` | ✅ | Replaces Build 06's stub. Logs every drifting item and throws — landing the job in `failed` is the alert (the Sentry message it also raised was removed by D49). Deliberately never auto-corrects the cache. Verified live: an injected un-ledgered mutation is detected and the cache is left untouched. |
