...
function Row({ label, value, bold, big }: { label: string; value: React.ReactNode; bold?: boolean; big?: boolean }) {
  return (
    <div
  ...
  return (
    ...
    >
            ...
            {b.lines.map((l, i) => {
              ...
              return (
                <tr key={i}>
                  ...
                  <td className="px-3 py-2.5 border-b border-border last:border-b-0 text-right">{fmt(l.client)}</td>
      ...
      <div className="flex flex-col mt-3.5">
        ...
        {b.recovery ? <Row label="Less material at site previously billed" value={"-" + fmt(b.recovery).slice(1)} /> : null}
      ...
      <div className="mt-3.5 p-3 border border-border rounded-lg">
        <div className="text-[11px] font-semibold tracking-wide uppercase text-muted-foreground mb-1.5">Bill copy</div>
        {b.files.length ? (
      ...
      {isMoney(role) && (
        <div className="mt-3.5 p-3 border border-dashed border-border-strong rounded-lg bg-muted/40">
          ...
          <Row
            ...
            value={
              <>
                {fmt(t.margin)} <span className="text-muted-foreground text-xs">({pct(t.margin, t.taxable)}%)</span>
              </>
            }
            bold
          />
