-- 0044 — fix 0042: ambiguous column references in rpc_create_bill
--
-- The aggregation query joins v_billable_now (aliased bn) against
-- jsonb_to_recordset(p_lines) (aliased sel), and sel carries its own
-- source_type/source_id columns by construction — every unqualified
-- reference to source_type/source_id/amount/internal_cost in the SELECT
-- list was therefore ambiguous ("column reference is ambiguous"), caught
-- immediately by this build's own first live smoke test, before anything
-- was built on top of it. Never edit an already-applied migration (AGENTS.md
-- database rule 1) — the whole function is recreated here, not altered in
-- place in 0042.
